print("*** ИГРЫ С ЧИСЛАМИ ***") #выводит текст на экран
print() 
#'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
while 1: #бесконечный цикл
    try: #проверка 
        a=abs(int(input("Введите целое число => "))) #abs-число будет всегда положительным
        break #остановка цикла
    except:
        ValueError
        print("Это не целое число")
#'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
if a==0: #проверка числа, является ли он нулем
    print("Нет смысла ничего делать с нулём")
else:
#'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    print("Определяем, сколько в числе чётных и сколько нечётных цифр")
    print()
    c=b=a #приравнивание значений переменных друг к другу 
    paaris=0
    paaritu=0
    while b > 0: 
        if b%2==0: #деление
            paaris += 1 
        else:
            paaritu += 1
        b = b // 10 #деление без остатка
    print("Чётных цифр:", paaris)
    print("Нечётных цифр:", paaritu)
    print()
#''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    print("*Переворачиваем* введённое число")
    print()
    b=0 #переменная
    while a > 0:
        number = a % 10
        a = a // 10
        b = b * 10
        b += number
    print("*Перевёрнутое* число", b)
    print()
#''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    print("Проверяем гипотезу Сиракуз")
    print()
    if c % 2 == 0: #деление
        print(с," - чётное число. Делим на 2.")
    else:
        print(c," - нечётное число. Умножаем на 3, прибавляем 1 и делим на 2.")
    while c != 1:
        if c % 2 == 0:
            c = c / 2
        else:
            c = (3*c + 1) / 2
            print(round(c), end=" ") #вывод итога гипотезы
    print()
    print("Гипотеза верна")
